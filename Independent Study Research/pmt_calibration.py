...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 12 11:59:38 2023
...
class drs4_data_processor(object):
    def __init__(self, folder_name):
        ...
        for path in pathlist:
            ...
            file = uproot.open(path_in_str)
            histogram = file["ha15"]  # assuming the histogram is named "histo"
            self.charge_histograms[self.voltage_settings[-1]] = histogram
...
class drs4_charge_histogram():
    def __init__(self,filename):
        ...
        tree = file.Get("myTree") # Replace "myTree" with the name of the TTree in your ROOT file

        charge = []
        events = []
        
        for event in tree:
            charge.append(-0.5*(event.channelA + event.channelB))
            events.append(event.event_number)

        charge = np.array(charge)
        events = np.array(events)/sum(events)

        self.number_of_events = len(events)
        self.calibration = pmt_calibration(charge, events)
